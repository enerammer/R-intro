...
```{r positron-analogy, echo = FALSE, fig.show = "hold", out.width = "100%", fig.alt = "Positron extends what R can do, and makes it easier to write R code and interact with R."}
knitr::include_graphics("./fig/r-manual.jpeg")
```


```{r positron-analogy-2, echo = FALSE, fig.show = "hold", fig.alt = "automatic car gear shift representing the ease of Positron", out.width="100%"}
...
Begin by downloading the dataset called ["`movie_series.csv`"](/data/movie_series.csv). 
...
You could also have installed the **`tidyverse`** packages by running this command directly in the R terminal.
...
- Use Positron to write and run R programs
